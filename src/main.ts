...
async function run(): Promise<void> {
  try {
    ...
    manifestText.replace('{{id}}', sha256);

    core.debug('setting sha256...');
    manifestText.replace('{{sha256}}', sha256);

    core.debug('setting url...');
    manifestText.replace('{{url}}', fullUrl);

    core.debug('setting version...');
    manifestText.replace('{{version}}', version.toString());
    manifestText.replace('{{version.major}}', version.toString(1));
    manifestText.replace('{{version.major_minor}}', version.toString(2));
    manifestText.replace('{{version.major_minor_patch}}', version.toString(3));
